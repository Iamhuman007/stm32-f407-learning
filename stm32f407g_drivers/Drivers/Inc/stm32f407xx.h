...
/*
 *   APRM CORTEX Mx Processor Priority Register Address Calculation
 */
...
#define SRAM1_BASEADDR                        0X20000000U // HAVE TO CHECK BASE ADDRESS
#define SRAM2_BASEADDR                        0X20001C00U
#define SRAM                                  SRAM1_BASEADDR
#define ROM_BASEADDR                          0X00000000U // SYSTEM MEMORY

/*
 * AHBX AND APBX Bus Peripheral base addresses
 * the peripheral base address starts from 0X40000000U and then divided among AHB1 AHB2 APB1 etc. if x to x+100 address is assigned to AHB1 then
 * certain peripherals like GPIO, ADC etc may be assigned to these addresses
 *
 * refer ppt or the memory map lecture + the reference manual to know which peripheral register is assigned to the address 0x40000000
 */

#define PERIPH_BASE                           0X40000000U// TIM CR1 register
...
/*no of register in a particular peripheral may change from MCU to MCU*/

typedef struct
{
	// volatile or __vo but we have to define it as a macro
	...
	volatile uint32_t AFR[2];// AN ARRAY to represent AFLR(low)ie AFR[0] and ARHR(High) AFR[1]
}GPIO_RegDef_t;
...
{
	// 26 dec
	__vo uint32_t IMR; // what are these registers study about them?
	// go through the rm009 files
	__vo uint32_t EMR;
	__vo uint32_t RTSR;
	__vo uint32_t FTSR;
	__vo uint32_t SWIER;
...
/*
 * peripheral register definition structure for SYSCFG(system config)
 */
typedef struct
{
...
#define GPIOA  				((GPIO_RegDef_t*)GPIOA_BASEADDR) // “Whenever I write GPIOA, treat the hardware memory starting at GPIOA_BASEADDR as a GPIO_RegDef_t structure.”

#define GPIOB  				((GPIO_RegDef_t*)GPIOB_BASEADDR)//  Treat the number GPIOA_BASEADDR as an address where  GPIO_RegDef_t object begins.
...
#define USART6_PCLK_EN() (RCC->APB1ENR |= (1 << 5)) // i think its apb2
...
// get these check if there is & instead of |??????
/*
 * Clock Disable Macros for SYSCFG peripheral
 */

/*
 * Macros to reset GPIOx peripherals  how does this work?
 *
 */                                                 // if multiple statement have to executed in a macro we use do{} while(0)
...
/* Bit position of macro of SPI peripherals */

//SPI_CR1 register
#define SPI_CR1_CPHA                     0
#define SPI_CR1_CPOL                     1
#define SPI_CR1_MSTR                     2
#define SPI_CR1_BR                       3 // 4 and 5 are for Baud Rate
